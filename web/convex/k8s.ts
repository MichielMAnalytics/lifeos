          ...
          initContainers: [
            ...
            },
            {
              ...
              command: ["sh", "-c", [
                ...
                'fi',
                '',
                '# Wipe any stale Google OAuth token files that older custom helpers',
                '# may have written. Calendar auth is centralised in LifeOS now;',
                '# the google-calendar skill instructs the bot to read tokens via',
                '# the LifeOS broker endpoint instead. Leaving these around lets',
                '# the bot fall back to a stale google-token.json after the user',
                '# reconnects on the dashboard.',
                'find /mnt/data -maxdepth 4 -type f \\( -name "google-token.json" -o -name "google-token-*.json" \\) -delete 2>/dev/null || true',
              ].join('\n')],
              env: [
