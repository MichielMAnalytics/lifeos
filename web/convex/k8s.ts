          ...
          initContainers: [
            ...
            },
            {
              ...
              command: ["sh", "-c", [
                ...
                'fi',
                '',
                '# Install / refresh LifeOS CLI from npm at every pod startup so we',
                '# pick up new commands (meeting/finance/skill/context) without a',
                '# Docker image rebuild. Failure here mustn\'t block the pod — the',
                '# image-baked CLI (if any) will still serve as fallback.',
                'mkdir -p /mnt/data/.npm-global',
                'npm config set prefix /mnt/data/.npm-global 2>/dev/null || true',
                'npm install -g lifeos-cli@latest --no-fund --no-audit 2>&1 | tail -5 || echo "[init] CLI auto-update failed; continuing"',
                '',
                ...
                'fi',
                '',
                '# Wipe stale Google calendar artefacts left by older self-bootstrapped',
                '# helpers. Calendar auth is centralised in LifeOS now; the',
                '# google-calendar skill instructs the bot to read tokens via the',
                '# LifeOS broker endpoint. Both raw token files AND any',
                '# google-calendar-config.json get removed so a stale tokenUrl can',
                '# never be the source of truth after a dashboard reconnect.',
                'find /mnt/data -maxdepth 4 -type f \\( -name "google-token.json" -o -name "google-token-*.json" -o -name "google-calendar-config.json" \\) -delete 2>/dev/null || true',
              ].join('\n')],
              env: [
  ...
  if (initSecretName) {
    ...
    initContainers.push({
      ...
      command: ["sh", "-c", [
        ...
        '# helpers. Calendar auth is centralised in LifeOS now; the',
        '# google-calendar skill instructs the bot to read tokens via the',
        '# LifeOS broker endpoint instead. Both must run on patch/restart',
        '# (not just on initial create) so existing pods get cleaned up.',
        'find /mnt/data -maxdepth 4 -type f \\( -name "google-token.json" -o -name "google-token-*.json" -o -name "google-calendar-config.json" \\) -delete 2>/dev/null || true',
        '',
        '# Fix stale broker URLs the bot baked into helper scripts before the',
        '# SKILL.md guidance was tightened. Targets any *.convex.site literal',
        "# without a region segment (e.g. 'charming-squid-23.convex.site') and",
        '# rewrites it to the live $LIFEOS_API_URL. New scripts the bot writes',
        '# from the updated skill should not need this; this is belt-and-',
        '# suspenders cleanup for existing pods.',
        'if [ -n "$LIFEOS_API_URL" ] && [ -d /mnt/data/.openclaw/workspace/scripts ]; then',
        '  for f in /mnt/data/.openclaw/workspace/scripts/*.js; do',
        '    [ -f "$f" ] || continue',
        '    sed -i "s|https://[a-z0-9-]*\\.convex\\.site|$LIFEOS_API_URL|g" "$f" 2>/dev/null || true',
        '  done',
        'fi',
        '',
        '# Plant a canonical broker-config helper so per-task scripts do not',
        '# have to know URLs at all. The bot has repeatedly tried to bake',
        '# literal URLs into config objects; this helper makes that pattern',
        '# unnecessary — any script that needs to call the broker can require',
        '# this one and let it source api_url + api_key from env or the',
        '# init-written config.json. Always overwrite on boot so a',
        '# bot-modified copy gets reset to a known-good shape.',
        'mkdir -p /mnt/data/.openclaw/workspace/scripts',
        "cat > /mnt/data/.openclaw/workspace/scripts/lifeos-broker.js <<'LIFEOS_BROKER_EOF'",
        '// Canonical LifeOS broker config loader. Planted by the LifeOS init',
        '// container on every pod boot — DO NOT edit; changes will be',
        '// overwritten on next restart. Scripts that call the broker should',
        "// `require('./lifeos-broker').loadBrokerConfig()` instead of building",
        '// a config object with hardcoded URLs.',
        "const fs = require('fs');",
        'function loadBrokerConfig(opts) {',
        ...
        '  }',
        '  if (!apiUrl || !apiKey) {',
        "    throw new Error('LifeOS broker not configured: set LIFEOS_API_URL/LIFEOS_API_KEY or provision ~/.lifeos/config.json');",
        '  }',
        "  const tokenUrl = apiUrl.replace(/\\/$/, '') + '/api/v1/google-calendar/access-token';",
        '  return {',
        "    calendarId: (opts && opts.calendarId) || 'primary',",
        '    tokenUrl,',
        ...
        '  };',
        '}',
        'module.exports = { loadBrokerConfig };',
        'LIFEOS_BROKER_EOF',
        '# Read-only mode is best-effort — same UID can chmod back, but it',
        '# discourages accidental overwrites by helper-generation flows.',
        'chmod 444 /mnt/data/.openclaw/workspace/scripts/lifeos-broker.js 2>/dev/null || true',
        '',
        '# Plant a canonical google-calendar-auth.js that wraps the broker',
        '# helper. Bots have repeatedly written add-calendar-event.js with',
        '# hardcoded wrong URLs in cfg.tokenUrl; this auth wrapper IGNORES',
        '# any cfg passed in and always reads broker config from env via',
        '# lifeos-broker. Net effect: even if the bot regenerates per-task',
        '# scripts with bad URLs in their cfg objects, the network call goes',
        '# to the right place because the auth layer overrides.',
        "cat > /mnt/data/.openclaw/workspace/scripts/google-calendar-auth.js <<'GCAL_AUTH_EOF'",
        '// Calendar auth helper. Planted by the LifeOS init container on',
        '// every pod boot — DO NOT edit; changes are overwritten on restart.',
        '// Always reads broker config from env via scripts/lifeos-broker.js,',
        '// ignoring any cfg.tokenUrl/tokenAuthHeader the caller may have',
        '// passed (bots have a habit of baking wrong URLs into cfg objects).',
        ...
        '',
        'function fetchRemoteToken(broker) {',
        '  return new Promise((resolve, reject) => {',
        "    const headers = { Accept: 'application/json', Authorization: broker.tokenAuthHeader };",
        '    https.get(broker.tokenUrl, { headers }, (res) => {',
        "      let data = '';",
        "      res.on('data', (chunk) => data += chunk);",
        "      res.on('end', () => {",
        '        if (res.statusCode === 409) {',
        "          const err = new Error('calendar disconnected — reconnect at app.lifeos.zone/settings');",
        "          err.code = 'calendar_disconnected';",
        '          return reject(err);',
        '        }',
        '        if (res.statusCode < 200 || res.statusCode >= 300) {',
        '          return reject(new Error(`Token fetch failed (${res.statusCode}): ${data}`));',
        '        }',
        '        try {',
        '          const parsed = JSON.parse(data);',
        '          if (!parsed.access_token || !parsed.expires_at_ms) {',
        "            return reject(new Error('Token response missing access_token or expires_at_ms'));",
        '          }',
        '          resolve(parsed);',
        '        } catch (e) {',
        '          reject(new Error(`Token response was not valid JSON: ${e.message}`));',
        '        }',
        '      });',
        "    }).on('error', reject);",
        '  });',
        ...
        '  const now = Date.now();',
        '  if (cachedToken && cachedExpiresAtMs - 60000 > now) return cachedToken;',
        '  const broker = loadBrokerConfig();',
        '  const token = await fetchRemoteToken(broker);',
        '  cachedToken = token.access_token;',
        '  cachedExpiresAtMs = token.expires_at_ms;',
        '  return cachedToken;',
        '}',
        '',
        'async function getCalendarClient(_cfg) {',
        '  // _cfg accepted for backward compat but its tokenUrl/tokenAuthHeader',
        '  // are intentionally ignored; auth source-of-truth is lifeos-broker.',
        ...
        '}',
        '',
        'module.exports = { getCalendarClient, getAccessToken };',
        'GCAL_AUTH_EOF',
        'chmod 444 /mnt/data/.openclaw/workspace/scripts/google-calendar-auth.js 2>/dev/null || true',
      ].join('\n')],
      env: [
