    ...
    case 'openclaw':
      return (
        <div className="space-y-4">
          ...
          <p className="text-xs text-text-muted/80">Then set your API key:</p>
...
Authentication: Bearer ${key}`}</CodeBlock>
    ...
    case 'codex':
      return (
        <div className="space-y-4">
          <p className="text-xs text-text-muted/80">Configure via the Codex CLI:</p>
          ...
          <button
            ...
            className={`flex items-center gap-2 rounded-full border px-4 py-2 text-sm transition-all duration-200 ${
              selected.has(a.id)
                ...
                : 'border-border/40 text-text-muted hover:border-border hover:text-text-muted'
            }`}
          >
