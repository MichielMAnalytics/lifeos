    ...
    <StepContainer backHref="/onboarding/plans">
      <div className="flex flex-col items-center text-center w-full max-w-md animate-fade-in">
        <h1 className="text-3xl font-light tracking-tight text-text sm:text-4xl">
          Enter your <span className="font-semibold">API key</span>
        </h1>
        <p className="mt-4 text-sm leading-relaxed text-text-muted max-w-sm">
          Your LifeCoach uses Claude under the hood. Connect your Anthropic API key to get started.
        </p>
        ...
        <div className="mt-8 grid grid-cols-2 gap-3 w-full">
          ...
          >
            <span className="absolute -top-2 left-1/2 -translate-x-1/2 rounded-full bg-accent px-2 py-0.5 text-[9px] font-medium text-bg tracking-wide whitespace-nowrap">Recommended</span>
            <span className="text-sm font-semibold text-text block">API Key</span>
            <span className="text-[11px] text-text-muted/70 block mt-1">From console.anthropic.com</span>
          </button>
          <button
          ...
          >
            <span className="text-sm font-semibold text-text block">Setup Token</span>
            <span className="text-[11px] text-text-muted/70 block mt-1">Via Claude Code CLI</span>
          </button>
        </div>
        ...
        <div className="mt-6 w-full text-left">
          {method === 'api_key' ? (
            <>
              ...
              />
              <div className="mt-3 rounded-xl border border-border/40 bg-surface/20 p-4 text-xs text-text-muted">
                <p className="font-medium text-text text-xs mb-2">How to get your API key</p>
            ...
            <>
              ...
              <div className="mt-3 rounded-xl border border-border/40 bg-surface/20 p-4 text-xs text-text-muted">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-md text-[10px] font-medium transition-colors ${hasClaudeCode ? 'bg-accent/15 text-accent' : 'text-text-muted/70 hover:text-text-muted'}`}
                    >Yes</button>
                    <button
                ...
                <p className="mt-3 text-[10px] text-text-muted/70">Uses your Claude Pro or Max subscription.</p>
              </div>
            </>
          )}
        </div>
